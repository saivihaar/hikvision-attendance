...
function Sync-HikEventsOnly {
    ...
    if ($withEmployee.Count -gt 0) {
        ...
        try {
            Invoke-CloudApi -Method POST -Path "/rest/v1/events?on_conflict=device_serial,employee_no,event_time,minor" -Body $payload -Prefer "resolution=ignore-duplicates,return=minimal" -ErrorAction Stop | Out-Null
        } catch {
